...
SUPPORT_ROOT = PurePosixPath(".harness/command_repo")
...
class CommandRepoAdapter(ObjectiveAdapter):
    name = "command_repo"

    def __init__(self) -> None:
        self._loaded_config_path: Path | None = None
        self._config: CommandRepoConfig | None = None
    ...
    def validate_args(self, args: argparse.Namespace) -> None:
        if args.adapter_config is None:
            raise SystemExit("--adapter-config is required with --adapter command_repo")
    ...
    def validate_repo(self, repo: Path, args: argparse.Namespace) -> None:
        ...
        missing = [
            repo / relative_path
            for relative_path in config.repo_required_paths
            if not (repo / relative_path).exists()
        ]
        if missing:
            joined = ", ".join(str(path) for path in missing)
            raise SystemExit(
                f"command_repo repo validation failed; missing required paths: {joined}"
            )
    ...
    def materialize_workspace(
        ...
        if config.benchmark_output_path is not None:
            benchmark_command = (
                f"{benchmark_command} (expects JSON at `{config.benchmark_output_path}`)"
            )
        ...
        return {
            ...
            "checkpoint_instructions": (
                "The current live repo files are the promoted candidate state. "
                f"To recover from a regression, restore from `{BASELINE_ROOT.as_posix()}` "
                "or the parent copies under `.harness/command_repo/parents/`."
            ),
            "context_files": ", ".join(format_paths(tuple(dict.fromkeys(context_files)))),
            "generation_summary": generation_summary,
            "additional_rules": (
                "Treat `.harness/command_repo/` as read-only reference material. "
                "Run the configured validation commands before claiming a valid candidate state, "
                "and only use the configured benchmark command to produce benchmark metrics."
            ),
        }
    ...
    def detect_candidate_state(self, spec: WorkspaceSpec) -> CandidateStateDetection:
        ...
        if changed:
            return CandidateStateDetection(
                status="ready",
                summary="benchmarking modified repo artifacts",
                has_state=True,
        ...
        return CandidateStateDetection(
            status="no_state",
            summary="no modified repo artifacts detected",
            has_state=False,
        )
    ...
    def benchmark(
        ...
        if not valid or objective_value is None:
            status = (
                "parse_failed"
                if parse_error is not None or not primary_metric_ok or missing_metrics
                else "failed"
            )
            return BenchmarkResult(
                status=status,
                summary=tail_text(spec.benchmark_log_path) or parse_error or "benchmark failed",
                objective_value=None,
                fitness=None,
                metrics=metrics,
    ...
    def _config_from_args(self, args: argparse.Namespace) -> CommandRepoConfig:
        if args.adapter_config is None:
            raise SystemExit("--adapter-config is required with --adapter command_repo")
        return self._load_config(args.adapter_config.resolve())

    def _load_config(self, path: Path) -> CommandRepoConfig:
        if self._loaded_config_path == path and self._config is not None:
            return self._config
        config = load_command_repo_config(path)
        self._loaded_config_path = path
        self._config = config
        return config

    def _require_config(self) -> CommandRepoConfig:
        if self._config is None:
            raise SystemExit(
                "command_repo adapter config is not loaded yet; pass --adapter-config and run setup first"
            )
        return self._config
    ...
    def _resource_instructions(self, gpu_id: int | None, requires_gpu: bool) -> str:
        ...
        return (
            ...
            f"`{gpu_id}`. Any manual validation or benchmark runs must use "
...
def load_command_repo_config(path: Path) -> CommandRepoConfig:
    try:
        payload = json.loads(path.read_text())
    except json.JSONDecodeError as exc:
        raise SystemExit(f"invalid command_repo adapter config JSON at {path}: {exc}") from exc
    if not isinstance(payload, dict):
        raise SystemExit(f"command_repo adapter config must be a JSON object: {path}")
    ...
    if required_metrics is None:
        ...
        if objective.primary_metric not in required_metric_paths:
            required_metric_paths = tuple(
                dict.fromkeys(required_metric_paths + (objective.primary_metric,))
            )

    repo_required_paths = payload.get("repo_required_paths")
    if repo_required_paths is None:
        default_required = [*editable_artifacts, *context_files]
    ...
    if not isinstance(requires_gpu, bool):
        raise SystemExit(f"command_repo config field requires_gpu must be a boolean: {path}")

    return CommandRepoConfig(
        objective=objective,
...
def expect_mapping(value: object, field_name: str, config_path: Path) -> dict[str, object]:
    if not isinstance(value, dict):
        raise SystemExit(f"command_repo config field {field_name} must be an object: {config_path}")
    return value
...
def expect_string(value: object, field_name: str, config_path: Path) -> str:
    if not isinstance(value, str) or not value.strip():
        raise SystemExit(
            f"command_repo config field {field_name} must be a non-empty string: {config_path}"
        )
    return value.strip()
...
def expect_direction(value: object, field_name: str, config_path: Path) -> str:
    ...
    if direction not in {"maximize", "minimize"}:
        raise SystemExit(
            f"command_repo config field {field_name} must be 'maximize' or 'minimize': {config_path}"
        )
    return direction
...
def expect_string_list(
    ...
    *,
    ...
    if not isinstance(value, list):
        raise SystemExit(f"command_repo config field {field_name} must be a list: {config_path}")
    items = tuple(expect_string(item, f"{field_name}[]", config_path) for item in value)
    if not allow_empty and not items:
        raise SystemExit(f"command_repo config field {field_name} must not be empty: {config_path}")
...
def expect_support_files(
    ...
    for index, item in enumerate(value):
        ...
        if not isinstance(include_as_context, bool):
            raise SystemExit(
                f"command_repo config field {field_name}[{index}].context must be a boolean: {config_path}"
            )
...
def expect_single_command(
    ...
    if not isinstance(value, list):
        raise SystemExit(
            f"command_repo config field {field_name} must be a command array: {config_path}"
        )
    command = tuple(expect_string(item, f"{field_name}[]", config_path) for item in value)
    if not command:
        raise SystemExit(
            f"command_repo config field {field_name} must not be an empty command: {config_path}"
        )
    return command
...
def normalize_relative_path(value: object, field_name: str, config_path: Path) -> str:
    ...
    if normalized.is_absolute() or normalized.parts[:1] == ("/",):
        raise SystemExit(
            f"command_repo config field {field_name} must be repo-relative, not absolute: {config_path}"
        )
    if ".." in normalized.parts or normalized == PurePosixPath("."):
        raise SystemExit(
            f"command_repo config field {field_name} must stay within the repo: {config_path}"
        )
    return normalized.as_posix()


def resolve_local_file(value: object, field_name: str, config_path: Path) -> Path:
    raw = expect_string(value, field_name, config_path)
    normalized = PurePosixPath(raw)
    if normalized.is_absolute() or normalized.parts[:1] == ("/",):
        resolved = Path(normalized.as_posix()).resolve()
    else:
        if ".." in normalized.parts or normalized == PurePosixPath("."):
            raise SystemExit(
                f"command_repo config field {field_name} must stay within the config directory: {config_path}"
            )
        resolved = (config_path.parent / Path(normalized.as_posix())).resolve()
    if not resolved.exists():
        raise SystemExit(
            f"command_repo config field {field_name} references a missing file: {resolved}"
        )
    if not resolved.is_file():
        raise SystemExit(
            f"command_repo config field {field_name} must point to a file: {resolved}"
        )
    return resolved
...
def parse_json_payload_from_text(
    text: str,
