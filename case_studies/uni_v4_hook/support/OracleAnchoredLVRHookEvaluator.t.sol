...
contract OracleAnchoredLVRHookEvaluator is Test, Deployers {
    ...
    function test_writesHarnessMetrics() public {
        ...
        metrics.serialize("fee_revenue_wad", feeMetrics.feeRevenueWad);
        metrics.serialize("unrecaptured_lvr_wad", feeMetrics.unrecapturedLvrWad);
        metrics.serialize("lp_net_from_toxic_flow_wad", feeMetrics.lpNetFromToxicFlowWad);
        metrics.serialize("recapture_ratio_bps", feeMetrics.recaptureRatioBps);
    ...
    struct FeeMetrics {
        ...
        int256 unrecapturedLvrWad;
        int256 lpNetFromToxicFlowWad;
        uint256 recaptureRatioBps;
    ...
    function _evaluateFeeGrid() internal returns (FeeMetrics memory metrics) {
        int24[10] memory gapTicks = [int24(10), 25, 50, 100, 150, 200, 300, 400, 600, 800];
        uint256 totalFeeUnits = 0;

        for (uint256 i = 0; i < gapTicks.length; ++i) {
            for (uint256 sign = 0; sign < 2; ++sign) {
                int24 signedGap = sign == 0 ? gapTicks[i] : -gapTicks[i];
                ...
                try hook.previewSwapFee(key, zeroForOne) returns (
                    ...
                    metrics.feeRevenueWad += FullMath.mulDiv(uint256(feeUnits) * 1e12, toxicNotionalWad, WAD);
        ...
        metrics.unrecapturedLvrWad = int256(metrics.grossLvrWad) - int256(metrics.feeRevenueWad);
        metrics.lpNetFromToxicFlowWad = int256(metrics.feeRevenueWad) - int256(metrics.grossLvrWad);
        metrics.recaptureRatioBps =
            metrics.grossLvrWad == 0 ? 0 : FullMath.mulDiv(metrics.feeRevenueWad, 10_000, metrics.grossLvrWad);
        metrics.avgToxicFeeUnits =
            metrics.executedToxicEventCount == 0 ? 0 : totalFeeUnits / metrics.executedToxicEventCount;
        metrics.capHitRateBps =
            metrics.toxicEventCount == 0 ? 0 : FullMath.mulDiv(metrics.capHitCount, 10_000, metrics.toxicEventCount);
    }
    ...
    function _evaluateWidthGrid() internal returns (WidthMetrics memory metrics) {
        ...
        metrics.admissionRateBps =
            metrics.scenarioCount == 0 ? 0 : FullMath.mulDiv(admittedCount, 10_000, metrics.scenarioCount);
    }
